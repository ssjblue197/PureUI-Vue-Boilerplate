...
import { FIREBASE_EVENTS } from '@/utils/constants';
    ...
    socket.current.on('internal_request', (data: any) => {
      ...
      if (
        ...
        Number(data.warehouse_id) ===
          Number(authStore.employee?.warehouse_id)
      ) {
        ...
        if (data.event_name) {
          switch (data.event_name) {
            ...
            case FIREBASE_EVENTS.EVENT_UPDATE_PRIORITY:
              EventBus.$emit(
                FIREBASE_EVENTS.EVENT_UPDATE_PRIORITY,
              );
              break;
            case FIREBASE_EVENTS.EVENT_REPORT_MISSING_BOX:
              EventBus.$emit(
                FIREBASE_EVENTS.EVENT_REPORT_MISSING_BOX,
              );
              break;
            case FIREBASE_EVENTS.DELETE_INTERNAL_REQUEST:
              EventBus.$emit(
                FIREBASE_EVENTS.DELETE_INTERNAL_REQUEST,
              );
              break;
            default:
              break;
