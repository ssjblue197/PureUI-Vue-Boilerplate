<template>
  <div class="wrapper">
    <div class="center relative w-[600px]">
      <p-table class="table" :options="options"></p-table>
    </div>
  </div>
</template>
...
const options = ref({
  columns: [
    ...
    },
    {
      ...
      headerName: 'Status',
      render: (row: RowData) =>
        `<p-tag variant="${row.status ? 'success' : 'danger'}" size="small">${row.status ? 'Active' : 'Inactive'}</p-tag>`,
    },
    {
      field: '',
      id: 'action',
